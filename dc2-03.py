...
# Jerome Caruso JavaScript Solution

# function getIt(arr) {
#     let smallestGuy = Infinity;
#     const letsGetIt = [];
#     arr.sort((a,b) => a-b)
#     for(let i = 1; i < arr.length; i++) {
#         const iAmSmall = Math.abs(arr[i] - arr[i - 1])
#         if(iAmSmall < smallestGuy) {
#             smallestGuy = iAmSmall;
#         }
#     }
#     for(let i = 1; i < arr.length; i++) {
#         const iAmSmall = Math.abs(arr[i] - arr[i - 1])
#         if(iAmSmall === smallestGuy) {
#             letsGetIt.push([arr[i -1], arr[i]])
#         }
#     }
#     return (letsGetIt);
# }

# console.log(getIt([4,2,1,3]))
# console.log(getIt([1,3,6,10,15]))
# console.log(getIt([3,8,-10,23,19,-4,-14,27]))